...
{
    public class ViewAlbums : ViewComponent
    {
        public ViewAlbums()
        {
        }

        public async Task<IViewComponentResult> InvokeAsync(List<AlbumViewModel> model,
            ...
            if (model != null)
            {
                if (!string.IsNullOrEmpty(searchCriteria))
                {
                    model = model.Where(s =>
                        ...
                        s.Place.ToLower().Contains(searchCriteria.ToLower()) ||
